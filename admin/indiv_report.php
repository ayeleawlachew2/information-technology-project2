...
$role=$_SESSION['role'];
$year=$_SESSION['year'];
$uname=$_SESSION['unanme'];
$qtype=$_SESSION['qtype'];
$login_time=$_SESSION['login_time'];
$logout_time="empty";
$totlarecordexist=mysqli_query("select*from candidate where year='$year' and unversity='$uname'");
...
$totalstatus1=0;
$totalstatus2=0;
$sql="select * from result WHERE program='$qtype'";
...
Graduate Student List Who Are Taken <?php echo($qtype);?> Exit Exam In  <?php echo $year; ?> E.C In <?php echo $uname;?>  
 </td></tr>	
<tr>
<th>Role Number</th>
<th>Frist Name</th>	
...
$candidateid=$row["uid"];	
		...
		$query="select * from candidate  where cid='$candidateid' and year='$year' and unversity='$uname'";
		$resultexist=mysqli_query($con, $query);
		if(mysqli_num_rows($resultexist)>0)
		{
		while($row2=mysqli_fetch_array($resultexist))
		{
$sql=mysqli_query("select*from user where uid='$candidateid'");
...
echo "No Candidate Registerd In $year year In  $uname Unversity";
?>	 
<br><br><br><br> 
  <br><br><br><br> 
    <br><br><br><br> 
