...
<head>
<title>Bloack All Candidate Account</title>
...
<fieldset class="fieldset"><legend>We can block all candidate account in the year</legend>
<br><br>
<div  >
<table>
    <form action="" method="post">
        ...
        <tr>
            <td>&nbsp;&nbsp;</td>
            <td><input type="submit" name="blockall" value="Block All Account"></td>
        </tr>
        <?php
            ...
            if($role == 'Candidate') {
                ...
                if(mysqli_num_rows($cand) > 0) {
                    while($row = mysqli_fetch_array($cand)) {
                        $cid = $row["cid"];
                        $update = mysqli_query($con, "UPDATE account SET status='inactive' WHERE uid='$cid'");
                    }
                    if($update != 0)	
